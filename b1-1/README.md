...
| 파일 | 설명 |
| --- | --- |
| `Dockerfile` | Ubuntu 24.04 기반 실행 환경, SSH/UFW/계정/권한/cron 구성 |
| `entrypoint.sh` | SSH, cron, UFW 설정 후 `agent-admin` 계정으로 앱 실행 |
| `monitor.sh` | 프로세스/포트/방화벽/자원 상태 점검 및 로그 기록 |
| `agent-app` | 제공된 x86_64 Linux 실행 바이너리 |
| `requirements-execution-report.md` | 요구사항 수행 내역서 |
| `evidence-checklist.md` | 필수 증거 자료 체크리스트 |
